...
class Generator(nn.Module):
    ...
    def forward(self, noise, class_label):
        """
        noise: shape of (batch_size, nz, 1, 1)
        class_label: shape of (batch_size, n_class, 1, 1)

        """

        cond_vector = torch.cat((noise, class_label), dim = 1)

        if cond_vector.is_cuda and self.ngpu > 1:
            output = nn.parallel.data_parallel(self.main, cond_vector, range(self.ngpu))
        else:
            output = self.main(cond_vector)
        return output
...
if __name__ == '__main__':    
    ...
    netD = Discriminator(ngpu, nc=1+10).to(device)
    ...
    for epoch in range(opt.niter):
        for i, (data, target) in enumerate(dataloader, 0):
            ...
            digit_target = torch.nn.functional.one_hot(target, n_class)
            ...
            G_target = torch.nn.functional.one_hot(target, n_class)
        ...
        if i % 100 == 0:
            vutils.save_image(real_data,
                    ...
                    normalize=True)
            fake = netG(fixed_noise, G_target)
